...
import { JITTER_MODE, type JitterMode } from "../../types/route.js";

export type ExponentialJitterMode = JitterMode;
...
export class ExponentialRetryStrategy implements RetryStrategy {
  ...
  public constructor(options: ExponentialRetryStrategyOptions) {
    ...
    this.jitter = options.jitter ?? JITTER_MODE.NONE;
  ...
  public nextDelayMs(attemptNumber: number): number {
    ...
    if (this.jitter === JITTER_MODE.FULL) {
      return this.random() * capped;
    }

    if (this.jitter === JITTER_MODE.EQUAL) {
      const half = capped / 2;
